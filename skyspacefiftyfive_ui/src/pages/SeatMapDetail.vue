					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateSeatMap()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2SeatMaps = true">Add</base-button>
								              <modal :show.sync="modal2SeatMaps">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Seat Map Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="SeatMapId" type="text" placeholder="Enter SeatMapId" v-model="seatMapDetails.seatMapId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Configuration</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Configuration" type="text" placeholder="Enter Configuration" v-model="seatMapDetails.configuration"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateSeatMap() {
      ...
      const jsonData = JSON.stringify(this.seatMapDetails);
      const res = await SeatMapService.update(jsonData);
...
//        this.getAllSeatMaps();
    ...
    async getSeatMapDetails() {
      try {
			let response = await SeatMapService.get(this.seatMapId);
			this.seatMapDetails = response.data;
