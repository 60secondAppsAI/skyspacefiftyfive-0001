					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateCustomerSupport()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2CustomerSupports = true">Add</base-button>
								              <modal :show.sync="modal2CustomerSupports">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Customer Support Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="CustomerSupportId" type="text" placeholder="Enter CustomerSupportId" v-model="customerSupportDetails.customerSupportId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Inquiry</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Inquiry" type="text" placeholder="Enter Inquiry" v-model="customerSupportDetails.inquiry"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Response</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Response" type="text" placeholder="Enter Response" v-model="customerSupportDetails.response"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2CustomerSupports = false;
    ...
    async updateCustomerSupport() {
      ...
      const jsonData = JSON.stringify(this.customerSupportDetails);
      const res = await CustomerSupportService.update(jsonData);
...
//        this.getAllCustomerSupports();
    ...
    async getCustomerSupportDetails() {
      try {
			let response = await CustomerSupportService.get(this.customerSupportId);
			this.customerSupportDetails = response.data;
 ...
 formatDate(dateString) {
  ...
  mounted() {
    this.getCustomerSupportDetails();
  },
  computed: {
  },
