        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Checkin Details</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalCheckinDetails = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalCheckinDetails">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add CheckinDetail</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="CheckinDetailId" type="text" placeholder="Enter CheckinDetailId" v-model="checkinDetailToAdd.checkinDetailId"></base-input>
  <base-input label="CheckinTime" type="text" placeholder="Enter CheckinTime" v-model="checkinDetailToAdd.checkinTime"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="checkinDetails" :row-key="record => record.CheckinDetailId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <CheckinDetailPictureView :checkinDetails="checkinDetails" />
          </div>
...
import CheckinDetailPictureView from './CheckinDetailPictureView.vue';


const checkinDetailsColumns = [
  "checkinDetailId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    CheckinDetailPictureView
  },

  data() {
    return {
      modalCheckinDetails: false,
        isTableView: true,

      columns: [
        {
          title: 'Checkin Detail Id',
		dataIndex: 'checkinDetailId',
          visible: true,
          scopedSlots: { customRender: 'checkinDetailId' },
          sorter: true
          //sorter: (a, b) => a.checkinDetailId - b.checkinDetailId,
          //sorter: (a, b) => a.checkinDetailId.localeCompare(b.checkinDetailId),
        },
        {
          title: 'Checkin Time',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} checkinDetails`,
      },
      ...
      checkinDetailsTable: {
        columns: [...checkinDetailsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'checkinDetailId',           // Column to sort by
  ...
  methods: {
    ...
    async renderCheckinDetailsTable() {
      ...
      let checkinDetailsTableData = [];
      for (let i = 0; i < this.checkinDetails.length; i++) {
        checkinDetailsTableData.push({
          id: i,
          checkinDetailId: this.checkinDetails[i].checkinDetailId,
          year: this.checkinDetails[i].year,
          date: this.checkinDetails[i].date,
          competitionId: this.checkinDetails[i].competitionId,
          checkinDetailId: this.checkinDetails[i].checkinDetailId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-checkin-details',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForCheckinDetailsChanged', this.searchQuery);
		//this.renderCheckinDetailsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalCheckinDetails = false;

      const currentDate = new Date().getTime();
      this.checkinDetailToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.checkinDetailToAdd);
      console.log(jsonData);
      
      const res = await CheckinDetailService.addCheckinDetail(jsonData);
